function  [] = AttitudeAnim(filename,q,qhat,l,w,d,t_span,t_anim);
%ODES  
...
% qhat = time history of estimate quaternion
...
fig_pos = [0 0 1.8*1280 1.2*1024]; % Set size of figure
...
sc_true.bottom = patch(Pt(1,5:8),Pt(2,5:8),Pt(3,5:8),'g','FaceAlpha',0.7); % plot bottom face
...
frame_bhat.z = plot3([0;0],[0;0],[0;axis_length],'k--','LineWidth',2);
...
%     waitbar(lv1/n_frames,animation_waitbar) % update waitbar
   [~, ind_curr] = min(abs(t_frames(lv1)-t_span)); % find closest data point to desired frame
    ...
    q_bhata_curr = transpose(qhat(ind_curr,:));
    C_ba_curr = quat2dcm(q_ba_curr'); % DCM for current frame
    C_bhata_curr = quat2dcm(q_bhata_curr'); % DCM for estimate frame
    ...
    set(frame_bhat.z,'XData',[0;C_bhata_curr(3,1)]*axis_length,'Ydata',[0;C_bhata_curr(3,2)]*axis_length,'ZData',[0;C_bhata_curr(3,3)]*axis_length)
